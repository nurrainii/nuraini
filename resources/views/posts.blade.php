  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-4 mb-3">
          <div class="card">
       <div class="position-absolute bg-dark px-3 py-2 " style="background-color: rgba(0, 0, 0, 0.7"> <a href="/blog?category={{ $post->category->slug }}"class="text-decoration-none" class="text-white text-decoration-none">{{ $post->category->name }}</a></div>
          @if ($post->image)
          <img src="{{ asset('storage/' . $post->image) }}"{{ $post->category->name }} alt="{{ $post->category->name }}" class="img-fluid">   
          @else
